...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='14.3';` | +0.1 (`14.2` → `14.3` → `14.4`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v81';` | +1 (`v80` → `v81`) |
...
`APP_VERSION` is also printed in player error messages (`(v14.3)`), which is how
