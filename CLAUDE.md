...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='14.8';` | +0.1 (`14.7` → `14.8` → `14.9`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v86';` | +1 (`v85` → `v86`) |
...
`APP_VERSION` is also printed in player error messages (`(v14.8)`), which is how
