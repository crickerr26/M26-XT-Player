...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='14.4';` | +0.1 (`14.3` → `14.4` → `14.5`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v82';` | +1 (`v81` → `v82`) |
...
`APP_VERSION` is also printed in player error messages (`(v14.4)`), which is how
