...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='14.2';` | +0.1 (`14.1` → `14.2` → `14.3`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v80';` | +1 (`v79` → `v80`) |
...
`APP_VERSION` is also printed in player error messages (`(v14.2)`), which is how
