...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='14.7';` | +0.1 (`14.6` → `14.7` → `14.8`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v85';` | +1 (`v84` → `v85`) |
...
`APP_VERSION` is also printed in player error messages (`(v14.7)`), which is how
