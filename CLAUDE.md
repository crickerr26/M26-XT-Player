...
|---|---|---|
| App version | `index.html` — `const APP_VERSION='15.2';` | +0.1 (`15.1` → `15.2` → `15.3`) |
| Service-worker cache | `sw.js` — `const CACHE = 'media26-shell-v90';` | +1 (`v89` → `v90`) |
...
`APP_VERSION` is also printed in player error messages (`(v15.2)`), which is how
